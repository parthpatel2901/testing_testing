...
int comparo_fName(const void *pA, const void *pB)
{
	...
	stuDA_t student_B = *((stuDA_t*) pB);   // Extracting student records
	
	char *fName_A = student_A.data.fName;
	char *fName_B = student_B.data.fName;   // getting given names of student
...
int comparo_grade(const void *pA, const void *pB)
{
	...
	float grade_B = student_B.data.grade;   //get students grades
		...
		const size_t array_size)
{
	...
	case gNamesort:
		{
		qsort(pHEAD, array_size, sizeof(stuRec_t), comparo_gName);  // dynamic array sorting using qsort()
		}
	break;             // fill the code here for Part a)
	case gradesort:
		{
		qsort(pHEAD, array_size, sizeof(stuRec_t), comparo_gName);  // here qsort() function is used for sorting of array
		}
	break;               // fill the code here for Part b)
	default:             
	;                  
	}
	return pHEAD;
